...
import { useFetchProjectDetails } from "@/hooks/use-project";
import { getProjectProgress } from "@/lib/public";

import { cn } from "@/lib/utils";
import type { Project, Task, TaskStatus } from "@/schema";
...
const ProjectDetails = () => {
    ...
    if (!projectId || !workspaceId) {
        return <div>Error: Project ID or Workspace ID is missing</div>;
    }
    
    const [isCreateTask, setIsCreateTask] = useState(false);
    const [taskFilter, setTaskFilter] = useState<TaskStatus | "All">("All");

    const { data, isLoading } = useFetchProjectDetails(projectId!) as {
        data: {
        tasks: Task[];
        project: Project;
        };
